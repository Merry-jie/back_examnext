...
# Import redoc pour la conversion
try:
...
try:
    ...
    Logger.warning("Bibliotheque QR code non disponible")
...
class PostgresDB:
    ...
    def get_status_message(self):
        ...
        elif self.connected and self.table_exists and not self.has_data:
            return "Table vide - Utilisation des donnees simulees"
        elif self.connected and self.table_exists and not self.schema_valid:
            return "Schema invalide - Utilisation des donnees simulees"
        elif self.connected and not self.table_exists:
            return "Table manquante - Utilisation des donnees simulees"
        else:
            return f"Mode simulation - {self.error_message}"
...
class MockDatabase:
    ...
    def get_status_message(self):
        return "Mode simulation - Donnees locales"
...
class ConnectionManager:
    ...
    def show_connection_result(self, success, message, db_status="", details=""):
        ...
        if success:
            color = [0.3,0.8,0.3,1]
            title_text = "CONNEXION REUSSIE"
        else:
            color = [0.9,0.3,0.3,1]
            title_text = "CONNEXION ECHOUEE"
        ...
        if db_status:
            if "reelle" in db_status:
                status_color = [0.3,0.8,0.3,1]
            elif "simulees" in db_status or "simulation" in db_status:
                status_color = [0.9,0.6,0.1,1]
            else:
                status_color = [0.8,0.8,0.3,1]
        ...
        if details and details != db_status:
            content.add_widget(Label(
                text=details,
                color=[0.8,0.8,0.9,1],
                size_hint_y=None,
...
class DocumentAnalyzer:
    """Service d'analyse de documents avec OCR et QR code (version simplifiee)"""
    
    def __init__(self):
        self.available = False  # Mode demo
    
    def analyze_document(self, document_path, item_id=None):
        """Simule l'analyse d'un document"""
        if item_id == 1 or item_id == 4 or item_id == 6:
            return {
                'success': True,
            ...
            return {
                ...
                'suspicious_words': ['modifie']
            }
        elif item_id == 3:
            return {
                'success': True,
                'text': 'Document recent',
                'qr_data': {
                    'nom': 'NOO OK',
                    'date': '2023-09-30',
                    'lieu': 'ZZZ',
            ...
            return {
                'success': True,
                'text': 'Document avec mots suspects',
                'qr_data': {
                    'nom': 'NO OK',
                    'date': '1995-03-20',
                    'lieu': 'VVV',
                ...
                'has_alterations': False,
                'suspicious_words': ['corrige', 'modifie']
            }
        else:
            return {
                'success': True,
    ...
    def extract_person_info(self, text):
        """Extrait les informations personnelles simulees"""
        if 'valide' in text:
            return {
                'nom': 'EEE',
                'prenom': 'OK',
...
class CompactItem(BoxLayout):
    ...
    def verifier_document(self, item_id, parent_popup):
        """Lance la verification du document"""
        app = App.get_running_app()
        app.db.get_item(item_id, lambda item: self.afficher_verification(item, parent_popup))

    def afficher_verification(self, item, parent_popup):
        """Affiche l'interface de verification"""
        # Fermer le popup parent
        if parent_popup:
            parent_popup.dismiss()
        
        # Creer le popup de verification
        verif_popup = None
        ...
        # Titre
        title = Label(
            text=f"VERIFICATION DOSSIER #{item['id']}",
            font_size='20sp',
            bold=True,
            color=App.get_running_app().theme_colors['accent'],
            size_hint_y=None,
        ...
        # Barre de progression PLUS GROSSE
        self.progress_bar = ProgressBar(
            max=100,
            value=0,
            size_hint_y=None,
            height=dp(30)  # Plus gros
        )
        content.add_widget(self.progress_bar)
        
        # Label de statut
        self.status_label = Label(
            text="Initialisation de l'analyse...",
            color=[1,1,1,1],
            size_hint_y=None,
            height=dp(30),
            font_size='14sp'
        ...
        # Zone de resultats (sera remplie apres analyse)
        self.result_area = ScrollableWithButtons(size_hint_y=0.5)
        ...
        self.result_area.set_content(self.result_container)
        content.add_widget(self.result_area)
        
        # Zone de boutons d'action (caches initialement) - TOUS EN VIOLET
        self.action_buttons = BoxLayout(size_hint_y=None, height=dp(50), spacing=dp(10))
        self.action_buttons.opacity = 0
        content.add_widget(self.action_buttons)
        
        # Ligne de separation avant le toggle switch
        ...
        # Espaceur a gauche pour aligner a droite
        ...
        # Lancer l'analyse en arriere-plan avec l'ID de l'item
        Thread(target=self._run_verification_simplifiee, args=(item, verif_popup)).start()

    def _run_verification_simplifiee(self, item, popup):
        """Version simplifiee de la verification pour les tests"""
        import time
        analyzer = DocumentAnalyzer()
        
        # Simulation des etapes
        etapes = [
            (10, "Analyse du document..."),
            (30, "Extraction des informations..."),
            (50, "Analyse du QR code..."),
            (70, "Verification des donnees..."),
            (90, "Comparaison avec la base..."),
            (100, "Analyse terminee")
        ]
        
        for value, text in etapes:
            time.sleep(0.3)  # Simulation de traitement
            Clock.schedule_once(lambda dt, v=value, t=text: self._update_progress(v, t))
        
        # Analyser le document avec l'ID de l'item
        doc_result = analyzer.analyze_document("", item['id'])
        doc_info = analyzer.extract_person_info(doc_result['text'])
        
        # Determiner si la validation reussit ou echoue
        current_year = datetime.now().year
        qr_ans = doc_result['qr_data'].get('ans', 0)
        age_doc = current_year - qr_ans
        age_valid = age_doc > 2  # VERIFICATION DE L'AGE DU DOCUMENT, PAS DE LA PERSONNE
        
        has_alterations = doc_result.get('has_alterations', False)
        has_suspicious = len(doc_result.get('suspicious_words', [])) > 0
        
        # Comparaisons
        comparisons = {
            'nom': {
                'bd': item['nom'],
                'document': f"{doc_info.get('prenom', '')} {doc_info.get('nom', '')}",
                'match': True  # Simplifie pour la demo
            },
            'date_naissance': {
                'bd': item['date_naissance'],
                'document': doc_info.get('date_naissance', ''),
                'match': True  # Simplifie pour la demo
            },
            'lieu_naissance': {
                'bd': item['lieu_naissance'],
                'document': doc_info.get('lieu_naissance', ''),
                'match': True  # Simplifie pour la demo
            }
        }
        
        all_matches = all([c['match'] for c in comparisons.values()])
        
        # Construire le resultat
        validation_result = {
            'success': all_matches and age_valid and not has_alterations and not has_suspicious,
            'errors': [],
            'comparisons': comparisons,
            'document_info': doc_info,
            'qr_info': doc_result['qr_data'],
            'db_info': {
                'id': item['id'],
                'nom': item['nom'],
        ...
        # Ajouter des erreurs si necessaire
        if not age_valid:
            validation_result['errors'].append(f"Document trop recent: {age_doc} an(s) (doit etre emis il y a plus de 2 ans)")
        if has_alterations:
            validation_result['errors'].append("Detection de ratures ou modifications suspectes")
        if has_suspicious:
            validation_result['errors'].append(f"Mots suspects detectes")
        if not all_matches:
            validation_result['errors'].append("Les informations ne correspondent pas")
        
        Clock.schedule_once(lambda dt: self._show_validation_result(validation_result, popup))
    ...
    def _show_validation_result(self, result, popup):
        """Affiche les resultats de validation"""
        self.result_container.clear_widgets()
        
        if result['success']:
            # Succes
            success_label = Label(
                text="VALIDATION REUSSIE SANS ERREUR",
                color=[0.3, 0.8, 0.3, 1],
                size_hint_y=None,
                height=dp(40),
                font_size='16sp',
            ...
            self.result_container.add_widget(success_label)
            
            # Ajouter les boutons d'action (tous en violet)
            self.action_buttons.clear_widgets()
            self.action_buttons.opacity = 1
            
            # Bouton Telecharger (violet)
            download_btn = Button(
                text="TELECHARGER",
                size_hint_x=0.33,
                background_color=[0,0,0,0]
            )
            with download_btn.canvas.before:
                Color(rgba=App.get_running_app().theme_colors['accent'])
                RoundedRectangle(pos=download_btn.pos, size=download_btn.size, radius=[dp(6),])
            download_btn.bind(on_release=lambda x: self._download_document(result))
            
            # Bouton Marquer comme lu (violet)
            mark_read_btn = Button(
                text="MARQUER LU",
                size_hint_x=0.33,
                background_color=[0,0,0,0]
            )
            with mark_read_btn.canvas.before:
                Color(rgba=App.get_running_app().theme_colors['accent'])
                RoundedRectangle(pos=mark_read_btn.pos, size=mark_read_btn.size, radius=[dp(6),])
            mark_read_btn.bind(on_release=lambda x: self._mark_as_read(result, popup))
            
            # Bouton Rejeter (violet)
            reject_btn = Button(
                text="REJETER",
                size_hint_x=0.33,
                background_color=[0,0,0,0]
            )
            with reject_btn.canvas.before:
                Color(rgba=App.get_running_app().theme_colors['accent'])
                RoundedRectangle(pos=reject_btn.pos, size=reject_btn.size, radius=[dp(6),])
            reject_btn.bind(on_release=lambda x: self._mark_as_flagged(result, popup))
            
            self.action_buttons.add_widget(download_btn)
            self.action_buttons.add_widget(mark_read_btn)
            self.action_buttons.add_widget(reject_btn)
            
        else:
            # Echec
            fail_label = Label(
                text="VALIDATION NON ACCEPTEE",
                color=[0.9, 0.3, 0.3, 1],
                size_hint_y=None,
                height=dp(40),
                font_size='16sp',
            ...
            self.result_container.add_widget(fail_label)
            ...
            if result['errors']:
                ...
                self.result_container.add_widget(errors_title)
                ...
                for error in result['errors']:
                    ...
                    self.result_container.add_widget(error_label)
            ...
            if result['comparisons']:
                ...
                self.result_container.add_widget(comp_title)
                
                # En-tete tableau
                header = BoxLayout(size_hint_y=None, height=dp(30))
                header.add_widget(Label(text="Champ", size_hint_x=0.25, color=[0.8,0.8,1,1]))
                header.add_widget(Label(text="Base", size_hint_x=0.25, color=[0.8,0.8,1,1]))
                header.add_widget(Label(text="Document", size_hint_x=0.25, color=[0.8,0.8,1,1]))
                header.add_widget(Label(text="QR", size_hint_x=0.25, color=[0.8,0.8,1,1]))
                self.result_container.add_widget(header)
                
                for field, comp in result['comparisons'].items():
                    row = BoxLayout(size_hint_y=None, height=dp(35))
                    
                    # Couleur selon match
                    color = [0.3,0.8,0.3,1] if comp['match'] else [0.9,0.3,0.3,1]
                    
                    row.add_widget(Label(text=field, size_hint_x=0.25, color=color))
                    row.add_widget(Label(text=comp['bd'][:15], size_hint_x=0.25, color=color))
                    row.add_widget(Label(text=comp['document'][:15], size_hint_x=0.25, color=color))
                    
                    # QR info correspondant
                    qr_val = ""
                    if field == 'nom' and 'nom' in result.get('qr_info', {}):
                        qr_val = result['qr_info']['nom']
                    elif field == 'date_naissance' and 'date' in result.get('qr_info', {}):
                        qr_val = result['qr_info']['date']
                    elif field == 'lieu_naissance' and 'lieu' in result.get('qr_info', {}):
                        qr_val = result['qr_info']['lieu']
                    
                    row.add_widget(Label(text=qr_val[:15], size_hint_x=0.25, color=color))
    ...
    def _mark_as_read(self, result, popup):
        """Marque le dossier comme lu et met a jour l'affichage"""
        item_id = result.get('db_info', {}).get('id')
        if item_id:
            app = App.get_running_app()
            ...
            self._show_confirmation("Dossier marque comme lu - Deplace vers l'onglet Lu")
            
            # Fermer le popup
            if popup:
                popup.dismiss()
            
            # FORCER LE RAFRAICHISSEMENT DE L'ONGLET COURANT
            main_screen = app.root.get_screen('main')
            
            # Attendre un peu que la BD soit mise à jour
            Clock.schedule_once(lambda dt: main_screen.force_refresh_current_tab(), 0.5)

    def _mark_as_flagged(self, result, popup):
        """Marque le dossier comme rejete et met a jour l'affichage"""
        item_id = result.get('db_info', {}).get('id')
        if item_id:
            app = App.get_running_app()
            
            # Mettre a jour la base de donnees
            app.db.update_flagged_status(item_id, True)
            app.db.update_read_status(item_id, False)
            
            # Afficher confirmation
            self._show_confirmation("Dossier rejete - Deplace vers l'onglet Marque")
    ...
    def _download_document(self, result):
        """Simule le telechargement du document"""
        self._show_confirmation("Document telecharge en local")
    ...
    def _show_confirmation(self, message):
        ...
        if hasattr(self, 'result_container'):
            ...
            self.result_container.add_widget(conf_label)
            Clock.schedule_once(lambda dt: self.result_container.remove_widget(conf_label) if conf_label in self.result_container.children else None, 2)

    def _get_demo_document_path(self):
        """Retourne un chemin de document de demo"""
        # Creer un fichier demo si necessaire
        demo_path = os.path.join(os.path.dirname(__file__), 'demo_document.pdf')
        if not os.path.exists(demo_path):
            with open(demo_path, 'w') as f:
                f.write("Document demo")
        return demo_path
...
class AuthScreen(Screen):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.password = "APL-I-E123"  # Mot de passe precis
...
# -------------------------------------------------------------------
# Module Systeme (Documentation de l'application)
# -------------------------------------------------------------------
class SystemApp(BoxLayout):
    ...
    def build_ui(self):
        ...
        modules_section = self._create_section(
            "MODULES DISPONIBLES",
            [
                "• Convertisseur: conversion de fichiers (PDF, DOCX, HTML, TXT)",
...
class ConverterApp(BoxLayout):
    def __init__(self, **kwargs):
        ...
        self.converter = Redoc() if REDOC_AVAILABLE else None
        self.build_ui()
    ...
    def build_ui(self):
        ...
        # Titre
        title = Label(
            text="CONVERTISSEUR DE FICHIERS",
            font_size='18sp',
            bold=True,
            color=App.get_running_app().theme_colors['accent'],
            size_hint_y=None,
        ...
        scrollable = ScrollableWithButtons(size_hint_y=0.8)
        
        content = BoxLayout(orientation='vertical', spacing=dp(15), size_hint_y=None, padding=dp(10))
        content.bind(minimum_height=content.setter('height'))
        
        file_btn = Button(text="📁 Choisir un fichier", size_hint_y=None, height=dp(50), background_color=[0,0,0,0])
        with file_btn.canvas.before:
            Color(rgba=App.get_running_app().theme_colors['accent'])
            RoundedRectangle(pos=file_btn.pos, size=file_btn.size, radius=[dp(8),])
        file_btn.bind(on_release=self.select_file)
        content.add_widget(file_btn)
        
        self.file_label = Label(text="Aucun fichier selectionne", size_hint_y=None, height=dp(30), color=[0.8,0.8,0.9,1])
        content.add_widget(self.file_label)
        
        format_layout = BoxLayout(size_hint_y=None, height=dp(50), spacing=dp(10))
        format_layout.add_widget(Label(text="Format de sortie:", size_hint_x=0.4, color=[0.9,0.9,1,1]))
        self.format_spinner = Spinner(text="PDF", values=("PDF","DOCX","HTML","TXT"), size_hint_x=0.6, 
                                      background_color=[0.2,0.2,0.3,1], color=[1,1,1,1])
        format_layout.add_widget(self.format_spinner)
        content.add_widget(format_layout)
        
        convert_btn = Button(text="CONVERTIR", size_hint_y=None, height=dp(50), background_color=[0,0,0,0])
        with convert_btn.canvas.before:
            Color(rgba=App.get_running_app().theme_colors['accent'])
            RoundedRectangle(pos=convert_btn.pos, size=convert_btn.size, radius=[dp(8),])
        convert_btn.bind(on_release=self.convert_file)
        content.add_widget(convert_btn)
        
        self.status_label = Label(text="Pret", size_hint_y=None, height=dp(30), color=[0.8,0.8,0.9,1])
        content.add_widget(self.status_label)
    ...
    def select_file(self, *args):
        self.status_label.text = "Fichier selectionne (simule)"

    def convert_file(self, *args):
        self.status_label.text = "Conversion en cours... (simule)"
        Clock.schedule_once(lambda dt: setattr(self.status_label, 'text', "Conversion terminee!"), 2)
...
class AIApp(BoxLayout):
    ...
    def build_ui(self):
        ...
        # Zone d'envoi de message agrandie
        ...
        # Zone de reponse avec scroll
        scrollable = ScrollableWithButtons(size_hint_y=0.45)
        ...
        self.output_label.bind(
            ...
            width=lambda *args: setattr(self.output_label, 'text_size', (self.output_label.width, None))
        )
        
        output_container = BoxLayout(orientation='vertical', size_hint_y=None)
        output_container.bind(minimum_height=output_container.setter('height'))
        output_container.add_widget(self.output_label)
        
        scrollable.set_content(output_container)
        self.add_widget(scrollable)
    ...
    def _call_api(self, message):
        ...
        for i, (q, r) in enumerate(self.history):
            if q == message and r == "...":
                self.history[i] = (q, result)
                break
        else:
            self.history.append((message, result))

        Clock.schedule_once(lambda dt: self.update_output_display())

    def update_output_display(self):
        full_text = ""
        for q, r in self.history:
            full_text += f"[Vous] : {q}\n\n[IA] : {r}\n\n{'-'*50}\n\n"
        self.output_label.text = full_text
...
class MainScreen(Screen):
            ...
            if hasattr(app.db, 'wait_for_connection'):
                ...
                if connected:
                    Clock.schedule_once(lambda dt: conn_manager.show_connection_result(
                        True, "Connexion etablie avec succes!", status_message, details
                    ))
                    self.db_ready = True
                else:
                    Clock.schedule_once(lambda dt: conn_manager.show_connection_result(
                        False, "Connexion impossible - Utilisation des donnees de secours", status_message, details
                    ))
                    self.db_ready = False
            else:
                # Mode simulation (MockDatabase)
                status_message = app.db.get_status_message() if hasattr(app.db, 'get_status_message') else "Mode simulation actif"
                details = app.db.get_status_details() if hasattr(app.db, 'get_status_details') else "Donnees simulees locales"
                Clock.schedule_once(lambda dt: conn_manager.show_connection_result(
                    True, "Mode simulation actif", status_message, details
                ))
                self.db_ready = True
    ...
    def add_nav_button(self, icon_path, callback):
        ...
        icon = Image(source=icon_path, size_hint=(None, None), size=(dp(25), dp(25)))
        btn.add_widget(icon)
    ...
    def show_module_list(self):
        try:
            ...
            grid.clear_widgets()
            modules = [
                {"name": "Convertisseur", "letter": "C", "glass_color": [0.63, 0.82, 1, 0.2]},
                {"name": "Note", "letter": "N", "glass_color": [1, 0.71, 0.76, 0.2]},
                {"name": "IA", "letter": "A", "glass_color": [0.85, 0.71, 1, 0.2]},
                {"name": "Systeme", "letter": "S", "glass_color": [0.69, 0.88, 0.9, 0.2]}
            ]
            btn_size = dp(150)
        ...
        BoxLayout:
            ...
            BoxLayout:
                ...
                Widget:
                    ...
                    canvas:
                        Color:
                            rgba: [0.8, 0.6, 1, 0.8]  # Ligne lumineuse
                        Line:
