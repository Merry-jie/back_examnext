...
class CompactItem(BoxLayout):
    ...
    def afficher_verification_simulation(self, item, parent_popup):
        ...
        # Garder le code de simulation existant
...
class ConverterApp(BoxLayout):
    ...
    def build_ui(self):
        ...
        info = Label(
            text="Module terminal - Version simplifiée",
            color=[1,1,1,1],
            size_hint_y=0.5
        )
    ...
    def refresh_list(self):
        ...
        for idx, note in enumerate(self.notes):
            ...
            with note_container.canvas.before:
                ...
                note_container.rect_bg = RoundedRectangle(pos=note_container.pos, size=note_container.size, radius=[dp(5),])
            ...
            note_container.bind(pos=update_note_bg, size=update_note_bg)

            # Titre de la note (clic pour ouvrir)
            title_btn = Button(
                text=f"{note['titre']}",
                size_hint_x=0.9,
                background_color=[0,0,0,0],
            ...
            with del_btn.canvas.before:
                ...
                RoundedRectangle(pos=del_btn.pos, size=del_btn.size, radius=[dp(5),])
            del_btn.bind(on_release=lambda x, i=idx: self.delete_note(i))

            note_container.add_widget(title_btn)
            note_container.add_widget(del_btn)

            self.list_layout.add_widget(note_container)
