...
# -------------------------------------------------------------------
# MODULE COMMANDE VOCALE (avec indicateur sonore)
# -------------------------------------------------------------------
class VoiceCommandApp(BoxLayout):
    ...
    def build_ui(self):
        ...
        # Indicateur sonore (cercle coloré)
        self.sound_indicator = Widget(size_hint=(None, None), size=(dp(50), dp(50)))
        with self.sound_indicator.canvas:
            Color(rgba=[0.5, 0.5, 0.5, 1])  # Gris par défaut
            self.indicator_ellipse = Ellipse(pos=self.sound_indicator.pos, size=self.sound_indicator.size)
        self.sound_indicator.bind(pos=self._update_indicator_ellipse, size=self._update_indicator_ellipse)
    ...
    def _update_indicator_ellipse(self, instance, value):
        self.indicator_ellipse.pos = instance.pos
        self.indicator_ellipse.size = instance.size
    ...
    def stop_listening(self, *args):
        ...
        # Remettre l'indicateur en gris
        Clock.schedule_once(lambda dt: self.update_indicator(0))

    def monitor_sound(self):
        """Mesure l'amplitude sonore en continu et met à jour l'indicateur"""
        ...
        while self.listening and self.stream:
            try:
                ...
                intensity = min(1.0, amplitude / 5000)  # Ajuster le seuil
                # Mettre à jour l'indicateur sur le thread principal
                Clock.schedule_once(lambda dt, i=intensity: self.update_indicator(i))
            except Exception as e:
                break

    def update_indicator(self, intensity):
        """Change la couleur de l'indicateur en fonction de l'intensité"""
        # Rouge (intensité forte) à vert (intensité faible)
        r = intensity
        g = 1.0 - intensity
        self.sound_indicator.canvas.clear()
        with self.sound_indicator.canvas:
            Color(rgba=[r, g, 0, 1])
            self.indicator_ellipse = Ellipse(pos=self.sound_indicator.pos, size=self.sound_indicator.size)
...
class MainScreen(Screen):
    ...
    def show_module_list(self):
        try:
            ...
            grid.clear_widgets()
            # Couleurs sombres (gris) pour les modules
            modules = [
                {"name": "Convertisseur", "letter": "C", "glass_color": [0.2, 0.2, 0.2, 0.4]},
                {"name": "Note", "letter": "N", "glass_color": [0.25, 0.25, 0.25, 0.4]},
                {"name": "IA", "letter": "A", "glass_color": [0.15, 0.15, 0.15, 0.4]},
                {"name": "Systeme", "letter": "S", "glass_color": [0.3, 0.3, 0.3, 0.4]},
                {"name": "Commande vocale", "letter": "V", "glass_color": [0.18, 0.18, 0.18, 0.4]}
            ]
            btn_size = dp(150)
