...
class CompactItem(BoxLayout):
    ...
    def afficher_verification_reelle(self, result, parent_popup):
        ...
        if result.get('has_alterations'):
            rature_label = Label(
                text="⚠️ Ratures détectées dans le document",
                color=[0.9,0.6,0.1,1],
                size_hint_y=None,
                height=dp(30)
            )
        ...
        if result.get('simulation'):
            sim_label = Label(
                text="(Mode simulation - bibliothèques d'analyse non disponibles)",
                color=[0.9,0.6,0.1,1],
                size_hint_y=None,
                height=dp(25),
                font_size='11sp'
    ...
    def afficher_verification_simulation(self, item, parent_popup):
        ...
        # Garder le code de simulation existant (simplifié ici)
        # Vous pouvez réutiliser le code de l'ancienne version ou faire une popup simple
        pass
...
# -------------------------------------------------------------------
# Module IA (première version - gardée)
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# Module IA (deuxième version - doublon, on garde la première)
# -------------------------------------------------------------------
# (Supprimé pour éviter le doublon)
...
class SystemApp(BoxLayout):
    ...
    def build_ui(self):
        ...
        voice_section = self._create_section(
            "COMMANDE VOCALE",
            [
                "• Cliquez sur 'Démarrer l'écoute' et parlez en français",
                "• Commandes possibles :",
                "   - 'ouvre le navigateur'",
                "   - 'ouvre le gestionnaire de fichiers'",
                "   - 'envoie un email à adresse disant message'",
                "   - 'éteins l'ordinateur' (avec confirmation)",
                "• Les actions sensibles demandent confirmation",
                "• Cliquez sur 'Arrêter / Quitter' pour désactiver le micro"
            ]
        )
...
# -------------------------------------------------------------------
# MODULE COMMANDE VOCALE (NOUVEAU)
# -------------------------------------------------------------------
class VoiceCommandApp(BoxLayout):
    ...
    def build_ui(self):
        ...
        scrollable = ScrollableWithButtons(size_hint_y=0.5)
        self.log_label = Label(
            text="Prêt. Cliquez sur 'Démarrer' et parlez.",
            size_hint_y=None,
            color=[0.9,0.9,1,1],
    ...
    def start_listening(self, *args):
        if not VOICE_AVAILABLE or not self.recognizer:
            self.log("Module vocal non disponible (bibliothèques manquantes).", color=[0.9,0.3,0.3,1])
            return
        if self.listening:
            return
        self.listening = True
        self.start_btn.disabled = True
        self.log("Écoute en cours... Parlez maintenant.")
        # Lancer l'écoute dans un thread pour ne pas bloquer l'interface
        Thread(target=self.listen_once).start()

    def stop_listening(self, *args):
        self.listening = False
        self.start_btn.disabled = False
        self.log("Micro désactivé.")
    ...
    def listen_once(self):
        ...
        try:
            ...
            Clock.schedule_once(lambda dt: self.log("Aucune commande détectée (délai expiré)."))
            self.stop_listening()
            return
        except Exception as e:
        ...
        try:
            ...
            Clock.schedule_once(lambda dt: self.log("Désolé, je n'ai pas compris."))
            self.stop_listening()
    ...
    def process_command(self, command):
        ...
        elif "éteins l'ordinateur" in cmd_lower or "arrête le pc" in cmd_lower:
            self.require_confirmation("shutdown", "Voulez-vous vraiment éteindre l'ordinateur ?")
        elif "redémarre" in cmd_lower or "restart" in cmd_lower:
            self.require_confirmation("restart", "Voulez-vous vraiment redémarrer ?")
        elif "efface" in cmd_lower or "supprime" in cmd_lower:
            self.log("Action non autorisée sans confirmation explicite.", color=[0.9,0.3,0.3,1])
            self.speak("Désolé, je ne peux pas effacer de fichiers sans confirmation.")
        else:
            self.log("Commande non reconnue. Essayez : 'ouvre le navigateur', 'envoie un email', etc.")
            self.speak("Commande non reconnue.")

        self.stop_listening()  # Après traitement, on arrête l'écoute (on peut aussi laisser le bouton pour réécouter)
    ...
    def handle_email_command(self, cmd):
        ...
        # Exemple simplifié : on suppose que l'utilisateur dit "envoie un email à [adresse] disant [message]"
        import re
        ...
        if match:
            ...
            self.log("Format attendu : 'envoie un email à adresse disant message'")
            self.speak("Veuillez préciser le destinataire et le message.")
    ...
    def send_email(self, to_addr, message):
        ...
        try:
            ...
            server.quit()
            self.speak("Email envoyé.")
            self.log(f"Email envoyé à {to_addr}")
        except Exception as e:
            self.log(f"Erreur envoi email : {e}")
            self.speak("Échec de l'envoi de l'email.")
    ...
    def show_module_list(self):
        try:
            ...
            modules = [
                ...
                {"name": "Commande vocale", "letter": "V", "glass_color": [0.9, 0.7, 0.3, 0.2]}  # Nouveau
            ]
            btn_size = dp(150)
